...
import PartialInstallS from '@site/docs/_partial/_install_s.mdx';

# 使用模板创建 Agent

本教程是一个端到端的实践指南，将引导您在 10 分钟内创建并部署一个完整的 Agent 应用。我们将使用 LangChain 框架构建一个支持代码执行的智能助手，该助手能够通过编写和运行代码来解决各种问题。
...
AgentRun 提供了开箱即用的项目模板，可以快速生成包含最佳实践的项目结构。
...
关于环境变量的更多信息，可以参考 [全局参数配置](../installation#全局参数配置)
